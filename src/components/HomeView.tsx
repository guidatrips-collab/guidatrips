            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <button 
                onClick={() => {
                  ...
                  const text = encodeURIComponent(settings?.whatsappGreeting || "Olá, Guida Trips! Quero falar com a equipe.");
                  window.open(`https://wa.me/${number}?text=${text}`, "_blank");
                }}
                className="px-6 py-4 bg-[#E8711A] hover:bg-white text-[#0D1B2A] font-accent text-xs font-bold tracking-widest uppercase transition-all rounded shadow-lg flex items-center justify-center gap-2.5 hover:scale-[1.02] cursor-pointer"
              >
                ...
                <span>{settings?.homeHeroBtnText || "Fale com nossa Equipe"}</span>
              </button>
              <button 
            ...
            <div className="flex items-center gap-6 pt-8 border-t border-white/10 max-w-md">
              {(settings?.homeHeroStats || [
                ...
                <React.Fragment key={idx}>
                  <div>
                    <span className={`font-serif text-2xl font-bold ${idx === 0 ? "text-[#E8711A]" : "text-white"}`}>{stat.value}</span>
                    <span className="font-sans text-[10px] text-zinc-400 block uppercase tracking-wider">{stat.label}</span>
                  </div>
                  {idx < 2 && <div className="w-px h-8 bg-white/10"></div>}
                </React.Fragment>
              ))}
            </div>
          </div>
